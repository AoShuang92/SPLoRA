...
def save_best_model(model, tokenizer, epoch, best_loss, current_loss, save_path="./llama2-lora-alpacapb-best1"):
...
def main():
    ...
    #randomly taking 1000 samples from alpaca dataset
    dataset_ = list(train_dataset)
    ...
    for i in range(len(sampled_data_pb)):
        new_pb["instruction"] = sampled_data_pb[i]["transcript"]
        new_pb["output"]= sampled_data_pb[i]["task_description"]
